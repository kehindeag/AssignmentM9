import employeesJson from "/js/modules/init.js";

let arrEmployees1 = [];

async function newemp() {
  try {
    const response = await employeesJson(); // replace fetchData() with the
    const emp = await response.employees;
    //arrEmployees1.push(...emp);
    arrEmployees.push(emp);
    for (let i of emp) {
      let employeeData = [];

      arrEmployees1.push([i.id, i.name, i.extension, i.email, i.title]);
    }
    return emp;
